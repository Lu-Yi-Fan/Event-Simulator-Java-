...
public class Simulate7 {
    ...
    public Simulate7(int numOfServer, List<Pair<Double,
    ...
    public static PQ<Event> makePq(ImList<Pair<Double, Supplier<Double>>> lst,
        ...
        for (Pair<Double, Supplier<Double>> pair : lst) {
            ...
            //add new input for arrive Event
    ...
    public String run() {
        ...
        while (newPq.isEmpty() == false) {
            ...
            if (event.eventType() == "ServeEvent" && findServer.getStatus() == "Serving") {
            ...
            //need to update the moment person is served
            } else if (event.eventType() == "ServeEvent" && findServer.getStatus() == "Resting") {
                //check if this event can execute check avail timing
                //if can -> execute if cannot create new serve event to be executed
                //find server from shop
                //can execute because server available time is > event time
                if (event.getTime() >= findServer.getAvailTime()) {
                    ...
                    //create new serve event
                    event = new ServeEvent(event.getCust(), findServer.getAvailTime(), findServer,
